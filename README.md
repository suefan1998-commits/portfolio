...
- `output/summaries/`：用于网站和默认 PDF 的摘要与章节精选。
...
- 入库阶段会生成 `output/content/` 全文归档，并为缺失作品生成 `output/summaries/` 摘要草稿，草稿状态默认为 `待审核`。
- 摘要草稿需要人工确认；把 summary 文件中的 `summary_status` 改为 `已确认` 后，网站和摘要版 PDF 才展示该作品的摘要与章节精选。
...
- 精选作品预览使用已确认 summary 的概述或章节精选；未确认时只显示“摘要待确认”和阅读原文入口。
...
- 摘要版 PDF 每篇作品单独开页，结构为：标题、媒体/日期/类型、阅读原文链接、概述、章节精选。
