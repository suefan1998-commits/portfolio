...
2. 入库完成后，AI 读取新生成的正文 Markdown，并参考现有已确认 summaries 的 keyline 与章节精选风格。
3. AI 在对话中展示生成的 `keyline` 和章节精选候选；如果文章没有章节划分，可以生成 `## 精选节选`；如果文章已有章节划分，只保留原文章章节结构，不自动添加 `## 精选节选`，也不创造新的节选分段。
4. 用户确认或修改后，才写入对应 `output/summaries/` 文件；未明确确认状态时，`summary_status` 保持 `待审核`。
5. summary 写入后，再按需运行 `./portfolio.sh site` 或 PDF 命令。
...
- 单篇文章页标题上方保留小字 `Published work`；文章页顺序为：`Published work`、标题、媒体信息、keyline、阅读原文、章节精选。
...
- 摘要版 PDF 每篇作品单独开页，结构为：标题、媒体/日期/类型、阅读原文链接、keyline、章节精选。
